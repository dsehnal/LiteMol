...
namespace LiteMol.Comparison.Base.Evd {
    "use strict";

    export type ColumnMajorMatrix = { data: number[], rows: number, columns: number }

    export namespace ColumnMajorMatrix {
        export function get(m: ColumnMajorMatrix, i: number, j: number) { return m.data[m.rows * j + i]; }
        export function set(m: ColumnMajorMatrix, i: number, j: number, value: number) { m.data[m.rows * j + i] = value; }
        export function add(m: ColumnMajorMatrix, i: number, j: number, value: number) { m.data[m.rows * j + i] += value; }

        export function reset(m: ColumnMajorMatrix) {
            for (let i = 0, _l = m.data.length; i < _l; i++) m.data[i] = 0.0;
        }

        export function create(columns: number, rows: number): ColumnMajorMatrix {
            return {
                data: <any>new Float64Array(columns * rows),
                columns,
                rows
            }
        }
    ...
    export namespace EvdCache {
        export function create(size: number): EvdCache {
            return {
                size,
     ...
     */
    export function compute(cache: EvdCache): void {
