...
namespace LiteMol.Core.Structure.Query.Algebraic {
    ...
    function atomProp<T extends DataTable>(index: (t: DefaultAtomTableSchema) => number[], table: (m: MoleculeModel) => T, value: (t: T) => any[]): Selector { return (ctx, i) => { let s = ctx.structure; return value(table(s))[index(s.atoms)[i]] }; }

    export const residueSeqNumber = atomProp(m => m.residueIndex, m => m.residues, t => t.seqNumber);
    export const residueName = atomProp(m => m.residueIndex, m => m.residues, t => t.name);
    export const elementSymbol = atomProp(m => m.indices, m => m.atoms, t => t.elementSymbol);
    export const atomName = atomProp(m => m.indices, m => m.atoms, t => t.name);
    export const entityType = atomProp(m => m.entityIndex, m => m.entities, t => t.type);
